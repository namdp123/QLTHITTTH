...
namespace QLTHI_TTTINHOC
{
    public class ExcelExport
    {
        ...
        public const string T_DanhMucKhoa = "DanhSach";
        public const string T_LichThiGV = "LichGV";
        #endregion
        ...
        private void BuildReplacerLichThi(ref Dictionary<string, string> pReplacer, string khoa, string phong, string mon, string ngaythi, string tietthi)
        {
            if (pReplacer != null)
            {
                pReplacer.Add("%KhoaHoc", khoa);
                pReplacer.Add("%PhongThi", phong);
                pReplacer.Add("%mamon", mon);
                pReplacer.Add("%ngaythi", ngaythi);
                pReplacer.Add("%tietthi", tietthi);
            }
        }
        private void BuildReplacerLichThiGV(ref Dictionary<string, string> pReplacer, string khoa, string tengv)
        {
            if (pReplacer != null)
            {
                pReplacer.Add("%KhoaHoc", khoa);
                pReplacer.Add("%TenGV", tengv);
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            switch (viewName)
            {
                ...
                case T_DanhMucKhoa:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("DanhSach.xls").ToArray();
                    break;
                case T_LichThiGV:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("LichGV.xls").ToArray();
                    break;
                #endregion
            }
            // Get stream
        ...
        public bool ExportLichThi(List<lichthi> dataSource, ref string fileName, bool isPrintPreview, string khoa, string phong, string mon, string ngaythi, string tietthi)
        {
            // Check if data is null
            ...
            BuildReplacerLichThi(ref replacer, khoa, phong, mon, ngaythi, tietthi);

            return OutSimpleReport(dataSource, replacer, "DanhSach", isPrintPreview, ref fileName);
        }
        public bool ExportLichThiGV(List<lichthiGV> dataSource, ref string fileName, bool isPrintPreview, string khoa, string tengv)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            BuildReplacerLichThiGV(ref replacer, khoa, tengv);

            return OutSimpleReport(dataSource, replacer, "LichGV", isPrintPreview, ref fileName);
        }
